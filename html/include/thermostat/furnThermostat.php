...
<html>
    ...
    <body id="light">
		<div id="wrapper">
			...
			<p>So at <?php echo "$time"?> the furnace will turn on.</p>
			<br>
			<?php
			//now we have to write this to external file
			$time_file = fopen("/var/www/html/include/thermostat/furn_time_specified.txt", "w") or die("Unable to open file!");
			fwrite($time_file, $time);
			fclose($time_file);

			$command = escapeshellcmd('python3 /var/www/html/include/thermostat/furn_time_script.py');
